...
class Database:
    ...
    def add_user(self, id: int, name: str, language: str = 'uz', phone: str = None, kirim: int = None, chiqim: int = None):
    ...
    def add_chiqim(self, summa: int, izoh: str, kategoriya: str, user_id: int):
        try:
            sql = """
            INSERT INTO Chiqim(summa, izoh, kategoriya, user_id) VALUES(?, ?, ?, ?)
            """
            self.execute(sql, parameters=(str(summa), izoh, kategoriya, user_id), commit=True)
            return True
        except Exception as e:
            print(f"DB error: {e}")
            return False

    def add_kirim(self, summa: str, izoh: str, kategoriya: str):
        try:
            sql = """
            INSERT INTO Kirim(summa, izoh, kategoriya) VALUES(?, ?, ?)
            """
            self.execute(sql, parameters=(summa, izoh, kategoriya), commit=True)
            return True
        except Exception as e:
            print(f"DB error: {e}")
            return False
